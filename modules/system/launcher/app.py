...
def list_hexpansion_apps(): # List apps loaded from hexpansion EEPROMS
    #print (scheduler.apps[1].hexpansion_apps)
    return None
...
class Launcher(App):
    def __init__(self):
        ...
        eventbus.on_async(HexpansionAppLauncherAddEvent, self._handle_hexpansion_app_add, self)
        eventbus.on_async(HexpansionAppLauncherRemoveEvent, self._handle_hexpansion_app_remove, self)
        eventbus.on_async(
    ...
    async def _handle_hexpansion_app_add(self, event: HexpansionAppLauncherAddEvent):
        ...
        print(f"Adding hexpansion on port {event.port} with name {event.name} to launcher")
    ...
    def update_menu(self):
        self.menu_items = self.list_core_apps() + list_user_apps() + self.hexpansion_apps
        if self.menu:
            self.menu._cleanup()
        self.menu = Menu(
            self,
